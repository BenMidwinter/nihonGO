/** Gojūon + dakuten/handakuten + common yōon */
const KANA = {
  hiragana: [
...
function buildDeck({ scripts, scope }) {
  let pool = [];
  if (scripts.includes("hiragana")) {
    pool = pool.concat(KANA.hiragana.map((k) => ({ ...k, script: "hiragana" })));
  }
  if (scripts.includes("katakana")) {
    pool = pool.concat(KANA.katakana.map((k) => ({ ...k, script: "katakana" })));
  }
  if (scope === "basic") pool = pool.filter((k) => BASIC_ROMAJI.has(k.romaji));
  else if (scope === "dakuten") pool = pool.filter((k) => k.char.length === 1);
  return pool;
}
